...
var link=function donutLink()
	{
			 function link(scope, el, attr)
			{
					...
					svg.attr({width: width+180, height: height+180});
					// group for arcs
				...
				scope.$watch('data', function(data){
				  if(!data){ return; }
					...
					texts.enter().append("text")
						...
						.text(function(d) { if(d.data != 0) return " "+d.data+" "; } );
			 ...
			 function link(scope, el, attr)
			{
				...
				scope.$watch('data', function(data){
				  if(!data){ return; }
					...
					width=d3.max(data,function(d) { return d.length;})*10+45;
...
var map = function mapLink()
{
			...
			scope.$watchGroup(['mapdata','datacities'], function(values){
					if(!values[0]){ return; }
					if(!values[1]) { return; }
					
					var cityData = d3.map();
					values[1].array.forEach(function(element,index,array){
						cityData.set(element.code, + element.data);
					});
					var maxValue = d3.max(cityData.values());
				    /*console.log("The maximum value is " + maxValue);*/
				    linearColorScale.domain([0.0, maxValue]);
				    paths=paths.data(values[0].features);

				    paths.enter()
				    .append("path")
				    .attr("class","map")
				    .attr("id",function(d){ return d.gm_code; })
				    .attr("d", path)
				    .on("click",function(){
				    		if( clicked === false){
						    	...
						    	blueRegion=d3.select(this);
		          				...
		          				scope.$parent.$apply(function(){
		            				scope.$parent.city=scope.$parent.optionsCity.filter(function(el,index,array){ return el.code === d.gm_code;  })[0];
				    ...
				    .on('mouseenter', function(d){
				    	...
				    	blueRegion.transition().ease("circle").duration(1).style("fill", "blue" );
          				scope.$parent.$apply(function(){
            				scope.$parent.city=scope.$parent.city=scope.$parent.optionsCity.filter(function(el,index,array){ return el.code === d.gm_code;  })[0];
            				scope.$parent.selectCity();
          				});
        			})
				    ...
				    .attr("d", path)
				   	.on("click",function(d){
				    		if( clicked === false){
			   					...
			   					paths.transition().style("fill", function(d) { return linearColorScale(cityData.get(d.gm_code)); });
						    	blueRegion=d3.select(this);
						   		blueRegion.transition().ease("circle").duration(1).style("fill", "blue");
		          				...
		          				scope.$parent.$apply(function(){
				   				...
				   				paths.on('mouseenter', function(d){
						    	paths.style("fill", function(d) { return linearColorScale(cityData.get(d.gm_code)); });
						    		blueRegion=d3.select(this);
							   		blueRegion.transition().ease("circle").duration(1).style("fill", "blue");
...
myApp.controller('MainCtrl', function($scope,$compile){
		...
		$scope.optionsCity=[];
			...
			$scope.changeScatter = function(){
					...
					$scope.scatterData={ xName: $scope.optionScatter1.name , yName: $scope.optionScatter2.name ,
						scatter: $scope.optionScatter1.array.map(function(node,i){ return {x: node,y: $scope.optionScatter2.array[i], STED: stedNameData[i][1], Naam: stedNameData[i][0] }; }) };
			}
  ...
  .await(function(err,mapData,data){
    	if(err){ throw err; }
    	data.pop();
    	mapData.features=mapData.features.filter(function(el,index,array){ return el.gm_code !== "GM0366" && el.gm_code !== "GM0412" && el.gm_code !== "GM0462" && el.gm_code !== "GM0463";  });
    	var kroon=data.filter(function(el,index,array){ return el.Code === "GM1911";  })[0];
		...
		for(i=0;i<length;i++){
			$scope.optionsCity.push({name: data[i].GM_NAAM, code: data[i].GM_CODE, id: i });
				...
				$scope.optionsMap= [
					{ name: 'Average number of addresses per km2',array: data.map(function(node){ return { code: node.Code ,data: node.OAD }; }) },
					{ name: 'Number of inhabitants',array: data.map(function(node){ return { code: node.Code, data: node.AANT_INW}; }) },
					{ name: 'Number of Men',array: data.map(function(node){ return { code: node.Code, data: node.AANT_MAN}; }) },
					{ name: 'Number of Women',array: data.map(function(node){ return { code: node.Code, data: node.AANT_VROUW }; }) },
					{ name: 'Number of inhabitants per km2',array: data.map(function(node){ return { code: node.Code, data: node.BEV_DICHTH }; }) },
					{ name: 'Number of households',array: data.map(function(node){ return { code: node.Code, data: node.AANTAL_HH}; }) },
					{ name: 'Number of cars',array: data.map(function(node){ return {code: node.Code, data: node.AUTO_TOT }; }) },
					{ name: 'Average number of people in all households',array: data.map(function(node){ return {code: node.Code, data: node.GEM_HH_GR }; }) },
					{ name: 'Number of cars per household',array: data.map(function(node){ return {code: node.Code, data: node.AUTO_HH }; }) },
					{ name: 'Number of cars per km2',array: data.map(function(node){ return {code: node.Code, data: node.AUTO_LAND }; }) },
					{ name: 'Number of company cars',array: data.map(function(node){ return {code: node.Code, data: node.BEDR_AUTO }; }) },
					{ name: 'Number of motorcycles',array: data.map(function(node){ return {code: node.Code, data: node.MOTOR_2W }; }) },
					{ name: 'Total land and water area in hectares',array: data.map(function(node){ return {code: node.Code, data: node.OPP_TOT }; }) },
					{ name: 'Land area in hectares',array: data.map(function(node){ return {code: node.Code, data: node.OPP_LAND }; }) },
					{ name: 'Water area in hectares',array: data.map(function(node){ return {code: node.Code, data: node.OPP_WATER }; }) }
				];
			
			stedNameData = data.map(function(node){ return [node.Naam,parseInt(node.STED)]; });

			$scope.optionsScatter= [
					{ name: 'Average number of addresses per km2',array: data.map(function(node){ return +node.OAD; }) },
					{ name: 'Number of inhabitants',array: data.map(function(node){ return +node.AANT_INW; }) },
					{ name: 'Number of Men',array: data.map(function(node){ return +node.AANT_MAN; }) },
					{ name: 'Number of Women',array: data.map(function(node){ return +node.AANT_VROUW; }) },
					{ name: 'Number of inhabitants per square kilometer',array: data.map(function(node){ return +node.BEV_DICHTH; }) },
					{ name: 'Number of households',array: data.map(function(node){ return +node.AANTAL_HH; }) },
					{ name: 'Number of cars',array: data.map(function(node){ return +node.AUTO_TOT; }) },
					{ name: 'Average number of people in all households',array: data.map(function(node){ return +node.GEM_HH_GR; }) },
					{ name: 'Number of cars per household',array: data.map(function(node){ return +node.AUTO_HH; }) },
					{ name: 'Number of cars per km2',array: data.map(function(node){ return +node.AUTO_LAND; }) },
					{ name: 'Number of company cars',array: data.map(function(node){ return +node.BEDR_AUTO; }) },
					{ name: 'Number of motorcycles',array: data.map(function(node){ return +node.MOTOR_2W; }) },
					{ name: 'Land water area',array: data.map(function(node){ return +node.OPP_TOT; }) },
					{ name: 'Land area in hectares',array: data.map(function(node){ return +node.OPP_LAND; }) },
					{ name: 'Water area in hectares',array: data.map(function(node){ return +node.OPP_WATER; }) }
				];	
			
			$scope.citiesData = $scope.optionsMap[0];

			$scope.optionsCity.sort(function(a,b) {return d3.ascending(a.name,b.name); });
